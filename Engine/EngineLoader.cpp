 ...
 * Copyright (C) 2007-2015,2018,2020-2026 Sean Echevarria
 *
...
EngineLoader::CreateEngine(const std::string & engineSettingsFile)
{
	...
	{
		size_t pos = engineSettingsFile.rfind("/");
		if (pos != std::_Meta_npos)
			mConfigFileDirectory = engineSettingsFile.substr(0, pos + 1);
	}
	...
	// been defined before the patch that is referencing the other
	{
		...
		int autoGeneratedPatchNumber = 1;
		GeneratePatchNumbersForDefaultNotePatches(autoGeneratedPatchNumber);
		GeneratePatchNumbers(pElem, autoGeneratedPatchNumber);
...
EngineLoader::LoadSystemConfig(TiXmlElement * pElem)
{
	...
	for ( ; pChildElem; pChildElem = pChildElem->NextSiblingElement())
	{
		...
		unsigned int ledActiveColor = UINT_MAX;
		unsigned int ledInactiveColor = UINT_MAX; // ignored
...
void
EngineLoader::GeneratePatchNumbers(TiXmlElement* pElem, int &autoGeneratedPatchNumber)
{
	for (; pElem; pElem = pElem->NextSiblingElement())
	{
		...
		{
			// [issue: 60] import patches from referenced config file
			if (pElem->ValueStr() == "importPatches")
			{
				std::string importPatchFileStr = pElem->GetText();
				if (std::_Meta_npos == importPatchFileStr.rfind(":"))
				{
					if (std::_Meta_npos == importPatchFileStr.rfind("..") ||
						std::_Meta_npos == importPatchFileStr.rfind("/"))
					{
						// relative path, or filename only
						importPatchFileStr = mConfigFileDirectory;
						importPatchFileStr.append(pElem->GetText());
					}
				}

				TiXmlDocument doc(importPatchFileStr);
				if (!doc.LoadFile())
				{
					std::string err = doc.ErrorDesc();
					if (mTraceDisplay)
						mTraceDisplay->Trace(std::format("Error loading config file: failed to load importPatch file {} {}\n", importPatchFileStr, err));
					continue;
				}

				TiXmlHandle hDoc(&doc);
				TiXmlElement* pElemInner = hDoc.FirstChildElement().Element();
				// should always have a valid root but handle gracefully if it doesn't
				if (!pElemInner)
				{
					if (mTraceDisplay)
						mTraceDisplay->Trace(std::format("Error loading config file: invalid config in importPatch file {}\n", importPatchFileStr));
					continue;
				}

				TiXmlHandle hRoot(nullptr);
				hRoot = TiXmlHandle(pElemInner);

				pElemInner = hRoot.FirstChild("patches").FirstChildElement().Element();
				GeneratePatchNumbers(pElemInner, autoGeneratedPatchNumber);
			}
			else if (mTraceDisplay)
...
void
EngineLoader::GeneratePatchNumbersForDefaultNotePatches(int &autoGeneratedPatchNumber)
{
...
EngineLoader::GenerateDefaultNotePatches()
{
	for (const auto &it : kMidiNoteValues)
	{
		...
		// setup patch led color
		{
			...
			unsigned int ledInactiveColor = LookUpColor("dynamic", "momentary", 0, UINT_MAX);
	...
	for ( ; pElem; pElem = pElem->NextSiblingElement())
	{
			 ...
			 childElem = childElem->NextSiblingElement())
		{
			...
			else if (patchElement == "SetDynamicChannelRandomVelocity")
			{
				...
				continue;
			else if (patchElement == "patchListItemName")
			{
				std::string patchStr;
				if (childElem->GetText())
					patchStr = childElem->GetText();

				int num = GetPatchNumber(patchStr);
				if (-1 != num)
				{
		...
		{
			unsigned int ledActiveColor = UINT_MAX;
			unsigned int ledInactiveColor = UINT_MAX;
			...
			else if (axeFxBlockId)
			{
				...
				if (-1 == ledActiveColor && axeFxBlockChannel < 6)
					ledActiveColor = LookUpColor("*", blockChannelStrs[axeFxBlockChannel], 1, UINT_MAX);

				if (-1 == ledInactiveColor && axeFxBlockChannel < 6)
					ledInactiveColor = LookUpColor("*", blockChannelStrs[axeFxBlockChannel], 0, UINT_MAX);
			}
			else if (-1 != patchName.find("x/y") || -1 != patchName.find("X/Y"))
			{
				if (-1 == ledActiveColor)
					ledActiveColor = LookUpColor("*", "axexy", 1, UINT_MAX);

				if (-1 == ledInactiveColor)
					ledInactiveColor = LookUpColor("*", "axexy", 0, UINT_MAX);
			}
			else if (-1 != patchName.find("a/b") || -1 != patchName.find("A/B"))
			{
				if (-1 == ledActiveColor)
					ledActiveColor = LookUpColor("*", "axeCh", 1, UINT_MAX);

				if (-1 == ledInactiveColor)
					ledInactiveColor = LookUpColor("*", "axeCh", 0, UINT_MAX);
			}
			...
			if (-1 == ledInactiveColor)
			{
				ledInactiveColor = LookUpColor(device, patchType, 0, UINT_MAX);
				if (-1 == ledInactiveColor)
				{
					ledInactiveColor = kFirstColorPreset;
	...
	for ( ; pElem; pElem = pElem->NextSiblingElement())
	{
			 ...
			 childElem = childElem->NextSiblingElement())
		{
				...
				childElem->ValueStr() == "PatchMap") // PatchMap was the old name for switch
			{
				...
				{
					// switch assignment by patch name
					std::string patchName;
					// QueryValueAttribute does not work with string when there are 
					// spaces (truncated at whitespace); use Attribute instead
